...
// 현재 슬롯에 표시되고 있는 캐릭터 ID를 기억 (교체 여부 판단용)
let currentP1CharId = null;
let currentP2CharId = null;

/**
 * [핵심 1] 캐릭터 UI 업데이트 (Update & Swap 방식)
...
function updateCharacterUI(bundleId, charData) {
    ...
    if (isNewCharacter) {
        // [A] 캐릭터가 바뀌었다면 -> Swap 연출 (Fade Out -> Change -> Fade In)
        
        // 1. 페이드 아웃
        charBox.classList.add('fade-out');

        setTimeout(() => {
            // 2. 데이터 교체 (애니메이션 없이 즉시 적용)
        ...
        // [B] 같은 캐릭터라면 -> Update (HP바 애니메이션 유지)
...
function applyCharData(charBox, data, animate) {
    const nameTag = charBox.querySelector('.name-tag'); // HTML 구조에 맞게 수정
    const hpBar = charBox.querySelector('.hp-bar-fill');
    
    if (nameTag) nameTag.innerText = data.name;
    // if (data.img) charBox.style.backgroundImage = `url(${data.img})`;
    ...
    if (hpBar) {
        ...
        if (!animate) {
            ...
            void hpBar.offsetWidth; // Reflow 강제 (즉시 적용)
...
/**
 * [핵심 2] 스킬 UI 재생성 (Delete & Recreate 방식)
 * @param {string} bundleId 
 * @param {object} skillData 
 * @param {number} power - 현재 위력
 * @param {number} coinPower - 코인 위력 (배지 표시용)
 */
function rebuildSkillUI(bundleId, skillData, power, coinPower) {
    ...
    if (!skillData) return; // 스킬 없으면 삭제만 하고 종료

    // 2. HTML 문자열 생성
    let coinsHtml = '';
    // 코인 개수만큼 생성 (coinList 길이에 따름)
    for(let i=0; i < skillData.coinCount; i++) {
        coinsHtml += `<div class="coin"></div>`;
    }

    const container = document.createElement('div');
    container.className = 'skill-container'; // 기본 opacity: 0 (CSS)
    ...
    // 3. DOM 추가
    bundle.prepend(container); // 캐릭터 박스 앞에 추가

    // 4. 등장 애니메이션
    requestAnimationFrame(() => {
        container.classList.add('visible'); // opacity: 1
    });
}
